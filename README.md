...
French is live today — and the **proficiency-aware conversation partner**, **listening comprehension**, and **one-click class remediation** described above all shipped recently, alongside an upgrade of the diagnostic engine to real Bayesian Knowledge Tracing. See **[ROADMAP.md](ROADMAP.md)** for the full recently-shipped list.

The next headline is **Automated Speech Recognition (ASR)**: real-time feedback on spoken production, building directly on the conversation partner so the platform can coach pronunciation and fluency, not just written accuracy. Alongside it, deepening the teacher analytics that surface where a whole class is getting stuck.

This is built solo, nights and weekends. In the short term, sponsorship covers the zero-fun parts — server hosting and the AI / text-to-speech API bills that features like the conversation partner and listening audio now incur — and buys the time to build the next thing properly instead of rushing it. **The ultimate goal is to transition to building these tools full-time**, ensuring long-term maintenance, faster feature delivery, and the ability to keep the core platform free for students and public schools.
